...
function startup() {
  ...
  fetch("http://localhost:3000/characters")
    .then(res => res.json())
    .then(json => createCharacters(json));
}
...
setTimeout(function titleDisappear() {
  ...
  muteText.innerHTML =
    '<i class="fa fa-volume-up" style="font-size:17px; vertical-align: center;"></i>';
}, 5000);
...
function createCharacters(json) {
  json.forEach(c => {
    store.characters.push(
      new Character(c.id, c.name, c.house, c.health, c.spells, c.image_url)
    );
  });
  characterSelect();
}
  ...
  charLists.forEach(charList => {
    for (let c of Character.all()) {
      ...
      charButton.addEventListener("click", event => {
        if (charList.id == "p1") {
          player1 = Object.assign(
            ...
            {
              div: document.querySelector("#p1-container"),
              gameDiv: document.getElementById("p1-game-details-title")
            }
          );
          ...
          player2 = Object.assign(
            ...
            {
              div: document.querySelector("#p2-container"),
              gameDiv: document.getElementById("p2-game-details-title")
            }
          );
      ...
      charButton.addEventListener("mouseover", event => {
        charList.parentElement.parentElement.querySelector(
          ".character-image"
        ).src =
          c.imageUrl;
      });
...
function characterProfile(player) {
  ...
  for (let type of spellTypes) {
    ...
    typeButton.addEventListener("click", event => {
      let spellList = player.spells.filter(
        spell => spell.category === type.toLowerCase()
      );
      ...
      for (let spell of spellList) {
        ...
        spellButton.addEventListener("click", event => {
          ...
          if (
            player1.moves.length === game.rounds &&
            player2.moves.length === game.rounds
          ) {
